...
public class SecThread extends Thread {
	...
	private HashMap<Integer,String> instrToMethMap;
	...
	public void run() {
		...
		this.edgeSet = new HashSet(navigationGraph.edgeSet());
	...
	public void testMethods() {
		...
		/**
			...
			modificare edgeset -> tenere conto di esecuzione precedente per eliminare linee gia coperte. 
				associare numeri dell'instrumentazione ai rispettivi metodi.

			*/
	        String randomNavigationMethod = Randomness.getInstance().choice(this.edgeSet);
			...
			if(isReliable(test)) {
				ArrayList<MethodTest> mListTemp = test.getMethodList();
					...
					String startingNodeT = "";
				for(int im = 0; im < mListTemp.size(); im++) {
					if(im!=0){
						methodString = listOfNavigationMethods.get(im-1);
						methodData = methodString.split("-");
						methodName = methodData[0];
						startingNodeT = methodData[1];
						String outerNodeT = methodData[2];
						if(startingNode==startingNodeT){
							outerNode = outerNodeT;
						}
						if(im==1){
							startingNode = methodData[1];
							outerNode = methodData[2];
						}
					}
					MethodTest x = mListTemp.get(im);
					//System.err.println("sono nel nodo: " + startingNode);
					try {
						Executable exec = x.getMethod();

						//System.err.println("sto eseguendo il metodo : " + methodName);
						//System.err.println("per poterlo eseguire dovrei stare nel nodo : " + startingNodeT + "\nParametri: ");
						for(int i=0;i<x.getParamUsed().length;i++){
							//System.err.println(i + " param: " + x.getParamUsed()[i]);
						}
					...
					//System.err.println("metodo eseguito, vado al nodo: " + outerNode);
					startingNode = outerNode;
					}catch(InvocationTargetException  e) {
						//do stuff or do nothing?
					//System.err.println("Methodo fallito rimango al nodo: " + startingNode);
			...
			try {
				...
				System.err.println("Something bad happened during the reset!");
			}
			...
			*/
			for (Integer instrId : test.getBranchCovered()) {
				String methToRemove = this.instrToMethMap.get(instrId);
				Iterator<String> iter = this.edgeSet.iterator();
				boolean halt = false;
				while(iter.hasNext()  && !halt){
					String temp = iter.next();
					if(temp.contains(methToRemove)){
						halt = true;
						iter.remove();
					}
				}
	...
	public void instantiateInstrumentedClass() {
		try {
			...
			//check the mapping
			//sm.printInstrToMethodMap();
			this.instrToMethMap = new HashMap<Integer, String>(sm.getInstrToMethodMap());
			...
			// TODO Auto-generated catch block
			System.err.println("Error: Security problem! compiling part secThread " + e);
		} catch (IllegalArgumentException e) {
			// TODO Auto-generated catch block
			System.err.println("Error: Argument inserted not correct! compiling part secThread " + e);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			System.err.println("Error: file In/Out failed! compiling part secThread " + e);
			//e.printStackTrace();
		}
	}
